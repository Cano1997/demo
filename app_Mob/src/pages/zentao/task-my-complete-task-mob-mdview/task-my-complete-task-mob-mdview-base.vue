...
<embed-view :className="{ 'view-container': true, 'default-mode-view': true, 'demobmdview9': true, 'task-my-complete-task-mob-mdview': true }">
    ...
    <template slot="content">
            ...
            loadAction="Get"
            createAction="Create"
            fetchAction="FetchMyCompleteTask" 
...
export default class TaskMyCompleteTaskMobMDViewBase extends Vue {
     ...
     */
    protected model: any = {
        srfTitle: '任务移动端多数据视图（我的已完成任务）（部件视图）',
        srfCaption: 'task.views.mycompletetaskmobmdview.caption',
        srfSubCaption: '',
        dataInfo: '',
        viewname:'task.mycompletetaskmobmdview',
        iconcls: '',
        icon: 'tasks'
    }
   ...
   /**
    * 工具栏 TaskMyCompleteTaskMobMDView 模型
    *
    * @type {*}
    * @memberof TaskMyCompleteTaskMobMDView
    */
   ...
   /**
    ...
    public openSearchform() {
      let search :any = this.$refs.searchformtaskmycompletetaskmobmdview;
      if(search){
          search.open();
      }
    }
...
<style lang='less'>
@import './task-my-complete-task-mob-mdview.less';
</style>